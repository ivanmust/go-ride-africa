...
interface UseDriverRideChatOptions {
  rideId: string | null;
  enabled?: boolean;
}
...
export const useDriverRideChat = ({ rideId, enabled = true }: UseDriverRideChatOptions) => {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error fetching driver chat messages:", error);
    } finally {
      setIsLoading(false);
    }
  ...
  const sendMessage = useCallback(
    async (messageText: string) => {
      ...
      try {
        ...
        console.error("Error sending driver chat message:", error);
        return false;
      } finally {
        setIsSending(false);
      }
  ...
  const markAsRead = useCallback(async () => {
    ...
    try {
      ...
      console.error("Error marking driver chat messages as read:", e);
    }
