...
const ProfilePage = () => {
  const { profile, updateProfile, refreshProfile, user } = usePassengerAuth();
  const { locations: savedLocations, isLoading: loadingLocations, saveLocation, deleteLocation, getLocationIcon } = useSavedLocations();
  ...
  const handleAddLocation = async () => {
    ...
    const data = await saveLocation({
      ...
      label: newLocationLabel || undefined,
    });
    if (data) {
      setNewLocationName('');
  ...
  const getLocationIconComponent = (label: string | null) => {
    switch (label) {
      case 'home':
